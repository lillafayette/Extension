const buttons = [
  ...
  chrome.runtime.sendMessage({ action: 'getState' }, response => {
    response.playing.forEach((isPlaying, index) => {
      ...
      sliders[index].value = response.volumes[index] * 100; // Update slider with volume value
    });
  });
    ...
    sliders.forEach((slider, index) => {
      chrome.storage.local.get([`volume${index}`], result => {
        if (result[`volume${index}`] !== undefined) {
          ...
          chrome.runtime.sendMessage({ action: 'setVolume', index, volume: result[`volume${index}`] });
        }
      });
    });
  });
